...
//===== SHORTCUT TEXTS IN THE JAVASCRIPT: ====//
...
msg.reply('check your inbox for your commands!');
   ...
   return msg.channel.send(pingembed);
       ...
       if (msg.content.startsWith(prefix + 'myavatar')){
        ...
        msg.reply('check your inbox for your profile picture!');
        }
       ...
       if (msg.content.startsWith(prefix + 'avatar')){
        ...
        msg.reply('user\'s profile picture sent to your inbox!');
   }
   ...
   if (msg.content.startsWith(prefix + 'stats')){
    let FooterHinami = [
        ...
        `${client.user.username} is accepting your support..\nTreat ${client.user.username} well he or she will haunt you.`
      ]
      try {
...
if (msg.content.startsWith(prefix + 'credits')){
    ...
    let ccreator = ("TeamCo7Clan Developments");
 
    let ccommunity = ("All friends & guests");
 
    let chosting = ("TeamCo7Clan Network");
 
    let bicon = client.user.displayAvatarURL;
    let embed = new Discord.RichEmbed()
            .setColor("#9B59B6")
            .setThumbnail(bicon)
            .addField("Developments:", ccreator)
            .addField("Ideas:", ccommunity)
            .addField("Hosting:", chosting)
...
if (msg.content.startsWith(prefix + 'invite')){
  ...
  "title": "Invite Bot",
  "url": "https://discord.me/teamco7clan",
  "author": {
  "name": "Bot is not for public use yet. Please join our Discord for more information ==> Use command **!socialmedia**"
  },
  "color": 3447003,
  "footer": {
  "text": "© Copyright TeamCo7Clan",
...
if (msg.content.startsWith(prefix + 'ban')) {
  const banlog = msg.guild.channels.find(channel => channel.name === 'logs_discordserver');
...
if (msg.content.startsWith(prefix + 'kick')){
 const kicklog = msg.guild.channels.find(channel => channel.name === 'logs_discordserver');
 const mod = msg.author;
if(!msg.member.roles.some(r=>["AC Management","Akelli Staff"].includes(r.name)) )
 return msg.reply("you don\'t have permission to use that!");
 let user = msg.guild.member(msg.mentions.users.first() || msg.guild.members.get(args[0]));
...
if(msg.content.startsWith(prefix + 'report')){
  ...
  return msg.reply("you don't have sufficient access to execute this command! \n Requirement: Management Team or Staff Member.");
    ...
    var logs = msg.guild.channels.find('name', 'logs_discordserver');
...
if (msg.content.startsWith(prefix + 'poll')){
  if(!msg.member.roles.some(r=>["Head Staff","Staff"].includes(r.name)) )
  return msg.reply("you don't have sufficient access to execute this command! \n Requirement: Management Team or Staff Member");
...
if (msg.content.startsWith(prefix + 'mute')){
    ...
    const modlog = msg.guild.channels.find(channel => channel.name === 'logs_discordserver');
    const mod = msg.author;
    let user = msg.guild.member(msg.mentions.users.first() || msg.guild.members.get(args[0]));
    if (!user) return msg.channel.send("Couldn't find user.")
    let reason = msg.content.split(" ").slice(2).join(" ");
    if (!reason) return msg.channel.send('Please specify a reason for muting this user!')
  ...
  if (!muteChannel) return msg.channel.send('**Please create a channel with the name `logs_discordserver`**')
  if (!muterole) {
        try {
...
if (!msg.member.hasPermission("KICK_MEMBERS")) return msg.reply("Not enough privileges");
let wUser = msg.guild.member(msg.mentions.users.first()) || msg.guild.members.get(args[0])
if (!wUser) return message.reply("Couldn't find them yo");
//if (wUser.hasPermission("MANAGE_ROLES")) return msg.reply("Ok done?");
...
let warnchannel = msg.guild.channels.find(`name`, "logs_discordserver");
...
if (msg.content.startsWith(prefix + 'setgame')) {
  if(!msg.member.roles.some(r=>["Administrator","Head Staff"].includes(r.name)) )
  return msg.reply("you don't have sufficient access to execute this command! \n Requirement: Administrator or Head Staff");
